...
class AWG:
    ...
    def reset(self):
        self.write_wait([':SOUR1:BURS OFF',     ':SOUR1:BURS OFF',
                         ':OUTP1 OFF',          ':OUTP2 OFF'])
...
class AWG4(AWG):
    ...
    def arbitrary(self, data01, block_len=6000, ch=1, sample_rate=1607600, amp=5, offset=2.5, wait_time=0.0001):
        self.write_wait(f':SOUR{ch}:APPL:USER {sample_rate/16384},{amp},{offset}')
        len_d = data01.shape[0]
        prefix = f':SOUR{ch}:DATA:DAC16 VOLATILE,'
        d_l = []
        block_num = int(np.ceil(len_d / block_len))
...
class generate_weight:
    def __init__(self, delay, peak_shape, sep):
        self.delay = delay
        self.peak_shape = peak_shape
        self.sep = sep
    
    def __call__(self, x, amp=1):
        x = x / amp
        res = np.zeros((x.shape[0], x.shape[1], self.peak_shape.shape[1]))
        for i in range(x.shape[-1]):
            res[:, :, self.peak_shape[1] == i] = x[..., i].reshape((x.shape[0], x.shape[1], 1))
        res = res * self.peak_shape[0]
        res = np.concatenate([res.reshape((res.shape[0], -1)), np.zeros((res.shape[0], self.sep))], axis=1)
        res = np.concatenate([np.zeros(self.delay), res.flatten()])
        return res
...
class DMD():
    def __init__(self, nbImg, ill_time=33278, pic_time=33334):
        ...
        self.DMD.Initialize()
        self.border_tblr = [170, 369, 790, 589]
        self.nbImg = nbImg
